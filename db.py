...
class Database:
    ...
    def insert_observation(self, obs):
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            obs["target"], obs["coordinates"], obs["duration"],
            obs["start_time"].isoformat(), obs["end_time"].isoformat(),
            obs["priority"], obs["wavelength"], obs["status"],
            obs.get("telescope"), datetime.now(timezone.utc).isoformat()
        ))
    ...
    def insert_history_entry(self, obs):
        self.execute("""
        INSERT INTO history (telescope, observation_id, target, completed_at, success, duration)
        VALUES (?, ?, ?, ?, ?, ?)
        """, (
            obs["telescope"], obs["id"], obs["target"],
            datetime.now(timezone.utc).isoformat(), obs["success"], obs["duration"]
        ))
