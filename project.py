...
class RealTimeTelescopeScheduler:
    def __init__(self, root):
        ...
        self.observations = []
        self.telescopes = [
            {"name": "Hubble Space Telescope", "lat": 28.5, "lon": -80.6, 
             "status": "Operational", "current_observation": None, 
             "capabilities": ["Optical", "UV"], "efficiency": 0.95,
             "success_count": 0, "failure_count": 0, "total_observation_time": 0},
            {"name": "James Webb Telescope", "lat": 5.1, "lon": -30.4, 
             "status": "Operational", "current_observation": None,
             "capabilities": ["Infrared"], "efficiency": 0.98,
             "success_count": 0, "failure_count": 0, "total_observation_time": 0},
            {"name": "Chandra X-ray Observatory", "lat": 28.5, "lon": -80.6, 
             "status": "Operational", "current_observation": None,
             "capabilities": ["X-ray"], "efficiency": 0.90,
             "success_count": 0, "failure_count": 0, "total_observation_time": 0}
        ]
        self.weather_data = {
            "cloud_cover": 0.0, 
    ...
    def create_observation_tab(self):
        ...
        ttk.Label(submit_frame, text="Coordinates (RA/DEC):").grid(row=1, column=0, sticky="w")
        self.coord_entry = ttk.Entry(submit_frame, width=30)
        self.coord_entry.grid(row=1, column=1, sticky="ew", pady=2)
        
        ttk.Label(submit_frame, text="Duration (min):").grid(row=2, column=0, sticky="w")
        ...
        self.end_entry.insert(0, (datetime.now() + timedelta(hours=2)).strftime("%Y-%m-%d %H:%M"))
        self.end_entry.grid(row=4, column=1, sticky="w", pady=2)
        
        ttk.Label(submit_frame, text="Priority:").grid(row=5, column=0, sticky="w")
        self.priority_combo = ttk.Combobox(submit_frame, values=["Low", "Medium", "High", "Critical"])
        ...
        self.wavelength_combo = ttk.Combobox(submit_frame, 
                                           values=["Optical", "Infrared", "UV", "X-ray", "Radio"])
        self.wavelength_combo.current(0)
        self.wavelength_combo.grid(row=6, column=1, sticky="w", pady=2)
        
        ttk.Button(submit_frame, text="Submit Request", command=self.submit_observation).grid(
        ...
        self.obs_tree = ttk.Treeview(list_frame, columns=("target", "priority", "status", "telescope"), 
        ...
        # Scrollbar
        scrollbar = ttk.Scrollbar(list_frame, orient="vertical", command=self.obs_tree.yview)
        ...
        self.new_priority_combo = ttk.Combobox(update_frame, values=["Low", "Medium", "High", "Critical"])
        self.new_priority_combo.current(1)
        self.new_priority_combo.pack(side="left", padx=5)
        
        ttk.Button(update_frame, text="Update Priority", command=self.update_priority).pack(side="left", padx=5)
        ttk.Button(update_frame, text="Delete Request", command=self.delete_observation).pack(side="left", padx=5)
    ...
    def create_schedule_tab(self):
        ...
        ttk.Button(control_frame, text="Generate Schedule", command=self.generate_schedule).pack(side="left", padx=5)
        ttk.Button(control_frame, text="Optimize with AI", command=self.optimize_schedule).pack(side="left", padx=5)
        ttk.Button(control_frame, text="Refresh View", command=self.update_schedule_display).pack(side="left", padx=5)
    ...
    def create_monitoring_tab(self):
        ...
        self.telescope_tree = ttk.Treeview(telescope_frame, 
        ...
        ttk.Button(control_frame, text="Maintenance Mode", command=self.set_maintenance_mode).pack(side="left", padx=5)
        ttk.Button(control_frame, text="Refresh Status", command=self.update_telescope_display).pack(side="left", padx=5)
        ...
        # Weather controls
        ttk.Button(weather_frame, text="Refresh Weather", command=self.fetch_weather).pack(side=tk.BOTTOM, pady=5)
    ...
    def create_comparison_tab(self):
        ...
        # Comparison frame
        comparison_frame = ttk.LabelFrame(tab, text="Telescope Performance Comparison", padding=10)
        ...
        # Refresh button
        ttk.Button(comparison_frame, text="Refresh Comparison", 
                  command=self.update_comparison_display).pack(side=tk.BOTTOM, pady=5)
    ...
    def update_loop(self):
        ...
        while True:
            try:
                ...
                if (datetime.now() - self.weather_data["timestamp"]).seconds > 300:  # 5 minutes
                    self.fetch_weather()
    ...
    def update_all_displays(self):
        ...
        self.update_status(f"Last update: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    def update_system_status(self):
        ...
        available_telescopes = [t for t in self.telescopes 
        ...
        if available_telescopes:
            ...
            pending_obs = [obs for obs in self.observations 
                          if obs['status'] == "Pending" and 
                          obs['start_time'] <= now <= obs['end_time']]
            ...
            if pending_obs:
                ...
                for telescope in available_telescopes:
                    for obs in pending_obs:
                        if (obs['status'] == "Pending" and 
                            obs['wavelength'] in telescope['capabilities']):
                            
                            # Check weather conditions for optical/UV observations
                            if obs['wavelength'] in ["Optical", "UV"] and self.weather_data['cloud_cover'] > 0.7:
    ...
    def fetch_weather(self):
        ...
        try:
            ...
            self.weather_data = {
                "cloud_cover": min(1.0, max(0.0, self.weather_data["cloud_cover"] + random.uniform(-0.2, 0.2))),
                "timestamp": datetime.now(),
                "wind_speed": max(0, self.weather_data["wind_speed"] + random.uniform(-5, 5)),
                "humidity": min(100, max(0, self.weather_data["humidity"] + random.uniform(-10, 10)))
            }
    ...
    def update_weather_display(self):
        ...
        # Add text info
        info_text = (f"Wind Speed: {wind_speed:.1f} km/h\n"
                    f"Humidity: {humidity:.1f}%\n"
                    f"Last Updated: {self.weather_data['timestamp'].strftime('%H:%M:%S')}")
        
        self.weather_ax.text(0.5, -0.2, info_text, ha='center', va='center', 
    ...
    def update_telescope_display(self):
        ...
        for i, telescope in enumerate(self.telescopes, 1):
            if telescope['current_observation']:
                ...
                values = (telescope['name'], telescope['status'], 
                         obs['target'], remaining_str)
            else:
    ...
    def set_maintenance_mode(self):
        ...
        if telescope['status'] == "Maintenance":
            telescope['status'] = "Operational"
            messagebox.showinfo("Info", f"{telescope['name']} returned to operational status")
        else:
    ...
    def generate_schedule(self):
        ...
        now = datetime.now()
        pending_obs = [obs for obs in self.observations 
                      if obs['status'] == "Pending" and obs['start_time'] <= now <= obs['end_time']]
        
        if not pending_obs:
            messagebox.showinfo("Info", "No pending observations within their time windows")
            return
        ...
        messagebox.showinfo("Success", f"Schedule generated with {len([o for o in self.schedule if o['status'] == 'Scheduled'])} new observations")
        self.update_schedule_display()
    ...
    def optimize_schedule(self):
        ...
        if self.weather_data['cloud_cover'] < 0.5:
            for obs in self.observations:
                if (obs['status'] == "Pending" and 
                    obs['start_time'] <= now <= obs['end_time'] and
                    len(self.schedule) < len(self.telescopes) * 3):  # Don't overload schedule
                    
                    # Find suitable telescope
                    for telescope in self.telescopes:
                        if (telescope['status'] == "Operational" and 
                            not telescope['current_observation'] and
                            obs['wavelength'] in telescope['capabilities']):
        ...
        self.schedule.sort(key=lambda x: (
            ...
            -x['duration']  # Longer observations first (more likely to miss window)
        ))
    ...
    def update_schedule_display(self):
        ...
        if not self.schedule and not any(t['current_observation'] for t in self.telescopes):
        ...
        self.schedule_text.insert(tk.END, f"Generated at: {datetime.now().strftime('%Y-%m-%d %H:%M')}\n")
        self.schedule_text.insert(tk.END, f"Weather Conditions: {self.weather_data['cloud_cover']:.0%} cloud cover\n\n")
    ...
    def draw_schedule_timeline(self):
        ...
        now_x = 50 + ((now - start_time).total_seconds() / total_seconds) * (canvas_width - 100)
        self.schedule_canvas.create_line(now_x, 20, now_x, canvas_height - 20, fill="red", dash=(2, 2))
        ...
        for i, telescope in enumerate(self.telescopes):
            y = 70 + (i * lane_height)
            self.schedule_canvas.create_text(20, y + lane_height/2, text=telescope['name'], anchor="e")
            
            # Draw current observation if any
            if telescope['current_observation']:
                obs = telescope['current_observation']
                start_x = 50 + ((obs['start_time_actual'] - start_time).total_seconds() / total_seconds) * (canvas_width - 100)
                end_x = 50 + ((obs['end_time'] - start_time).total_seconds() / total_seconds) * (canvas_width - 100)
                
                # Ensure at least some width for very short observations
                end_x = max(end_x, start_x + 5)
                
                self.schedule_canvas.create_rectangle(
                    start_x, y + 5, end_x, y + lane_height - 5,
        ...
        for obs in self.schedule:
            if obs['status'] == "Scheduled" and obs['telescope']:
                ...
                start_x = 50 + ((obs['start_time'] - start_time).total_seconds() / total_seconds) * (canvas_width - 100)
                end_x = 50 + ((obs['end_time'] - start_time).total_seconds() / total_seconds) * (canvas_width - 100)
    ...
    def update_history_display(self):
        ...
        for entry in sorted(self.history, key=lambda x: x['completed_at'], reverse=True)[:10]:
            status = "SUCCESS" if entry['success'] else "FAILED"
            color = "green" if entry['success'] else "red"
            
            self.history_text.insert(tk.END, f"Telescope: {entry['telescope']}\n")
            self.history_text.insert(tk.END, f"Observation: {entry['observation']['target']}\n")
            self.history_text.insert(tk.END, f"Status: ")
            self.history_text.insert(tk.END, f"{status}\n", color)
            self.history_text.insert(tk.END, f"Completed: {entry['completed_at'].strftime('%Y-%m-%d %H:%M')}\n")
            self.history_text.insert(tk.END, f"Duration: {entry['observation']['duration']} minutes\n")
    ...
    def update_stats_display(self):
        ...
        telescope_usage = {t['name']: sum(1 for h in self.history if h['telescope'] == t['name']) 
    ...
    def update_comparison_display(self):
        ...
        for telescope in self.telescopes:
            ...
            observation_counts.append(telescope['success_count'] + telescope['failure_count'])
            observation_times.append(telescope['total_observation_time'])
        
        # Plot success rates
        bars = self.comp_ax1.bar(telescopes, success_rates, color=['blue', 'green', 'orange'])
        ...
        for bar in bars:
            ...
            self.comp_ax1.text(bar.get_x() + bar.get_width()/2., height,
                             f'{height:.1%}',
                             ha='center', va='bottom')
        ...
        bars1 = self.comp_ax2.bar(x, observation_counts, width, label='Observation Count')
        bars2 = self.comp_ax2.bar([i + width for i in x], observation_times, width, label='Total Observation Time (min)')
        ...
        for bar in bars1 + bars2:
            height = bar.get_height()
            self.comp_ax2.text(bar.get_x() + bar.get_width()/2., height,
                             f'{int(height)}',
                             ha='center', va='bottom')
